...
def programmatic_signed_doc(certification_id, user_email_address, file_1, psql):
    ...
    try:
        ...
        if company is not None:
            cur.execute(f"SELECT id, invoice_company_name, user_email_address, signed_agreement, application_date from cbi_programmatic_signed_agreement WHERE invoice_company_name='{company}'")
            data = cur.fetchone()
...
def single_signed_doc_get(certification_id, user_email_address, psql):
    ...
    try:
        ...
        cur.execute(
            f"SELECT id, certification_id, user_email_address, signed_agreement, application_date from cbi_single_signed_agreement WHERE user_email_address='{user_email_address}' and certification_id='{certification_id}'")
        data = cur.fetchone()
...
def programmatic_signed_doc_get(certification_id, user_email_address, psql):
    ...
    try:
        ...
        if company is not None:
            cur.execute(
                f"SELECT id, invoice_company_name, user_email_address, signed_agreement, application_date from cbi_programmatic_signed_agreement WHERE invoice_company_name='{company}'")
            data = cur.fetchone()
...
def verifier_signed_doc(certification_id, user_email_address, file_1, psql):
    ...
    try:
        ...
        if company is not None:
            ...
            if len(signed_agreement) > 0:
                if data is not None:
                    cur.execute(
                        f"UPDATE cbi_programmatic_signed_agreement SET signed_agreement='{signed_agreement}',application_date='{now}' WHERE invoice_company_name='{company}'; ")
                    con.commit()
                    return {'certificationId': certification_id, 'userEmail': user_email_address, "signedCertificationAgreement": file_1}, 200
                elif data is None:
...
def verifier_signed_doc_get(certification_id, user_email_address, psql):
