...
def assigned_queue(user_email_address, psql):
    ...
    try:
        ...
        cur.execute(
            f"select id, certification_id, certification_type, certification_status, reviewer, application_date, assigned_date, approved_date, user_company, certification_company, instrument_type, underwriter from cbi_certification_queue where reviewer='{user_email_address}' ;")
        ...
        if cert_queue is not None:
            for i in cert_queue:
                if i[2] == 'pre':
                    cur.execute(
                        f"select id, certification_id, user_email_address, certification_status, instrument_type, da_name, da_issuance_country, da_cusip, da_isin, da_local_currency_lc, da_amount_issued_lc, da_coupon, da_underwriter, da_issue_date, da_maturity_date, da_instrument_type, d_renewable_energy, d_renewable_energy_text, ps_financing_asset, ps_proceeds_allocation, pe_portfolio_approach, pe_assessment_procedure, pm_proceed_type, pm_proceed_detail, pm_proceed_timing, pm_proceed_use, ar_report_interval, ar_report_format, ar_report_access, ar_report_link, ar_report_breakdown, ir_report_interval, ir_report_format, ir_report_access, ir_report_link, ir_report_indicators, ci_address_head_office, ci_vat_number, ci_business_reg_number, cp_name, cp_position, cp_company, cp_contact_number, id_name, ca_application_date, ca_legal_name_issuing_entity, ca_unique_name_debt_instruments, ca_address, ca_email_address, ca_contact_person, ca_signature, single_issuer_agreement, verifier_agreement from cbi_pre_issuance_certification where certification_id='{i[1]}' ;")
        ...
        cur.execute(f"select work_notes,last_modified_date from cbi_reviewer_workspace  WHERE user_email_address='{user_email_address}';")
        ...
        con.close()
        return {'assignedCertifications': queue_main_data, 'workSpace':{'notes':workspace[0],'lastModified':workspace[1]} }, 200
...
def reviewer_workspace(user_email_address, notes, psql):
    ...
    try:
        ...
        try:
            ...
            cur.execute(f"SELECT id, user_email_address, work_notes, last_modified_date from cbi_reviewer_workspace  WHERE user_email_address='{user_email_address}'")
            data = cur.fetchone()
            con.commit()

            if data is None:
                query = "INSERT INTO cbi_reviewer_workspace(user_email_address,work_notes,last_modified_date) VALUES('{0}', '{1}', '{2}'); ".format(user_email_address,notes,now)
                cur.execute(query)
                con.commit()
            else:
                query = f"UPDATE cbi_reviewer_workspace SET work_notes='{notes}',last_modified_date='{now}' WHERE user_email_address='{user_email_address}'; "
                cur.execute(query)
                con.commit()
